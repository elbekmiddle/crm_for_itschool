...
const ExamSession: React.FC = () => {
  ...
  useEffect(() => {
    const init = async () => {
       try {
          const { data } = await api.get(`/exams/attempt/${attemptId}/questions`);
  ...
  const handleAnswer = (optIdx: number) => {
    ...
    autosaveRef.current = setTimeout(async () => {
       try {
          await api.post(`/exams/attempt/${attemptId}/answer`, { questionId: qId, answer: optIdx });
       } catch (e) {}
    }, 1000);
  };
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center font-black text-slate-400 animate-pulse">SESSIYA TAYYORLANMOQDA...</div>;
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col pt-20">
      ...
      <div className="fixed top-0 left-0 right-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800 z-40 px-8 py-3 flex justify-between items-center">
          ...
          <button onClick={handleFinalConfirm} className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-6 py-2.5 rounded-2xl font-black hover:scale-105 active:scale-95 transition-all flex items-center gap-2">
      ...
      <div className="max-w-5xl w-full mx-auto grid grid-cols-1 md:grid-cols-4 gap-6 p-6 flex-1">
          ...
          <div className="md:col-span-1 bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-sm border border-slate-200 dark:border-slate-800 self-start sticky top-24">
             <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Savollar ro'yxati</h3>
                ...
                {questions.map((q, idx) => (
                  <button 
                    ...
                    className={`aspect-square rounded-xl font-black text-sm transition-all ${
                       currentIdx === idx ? 'bg-indigo-600 text-white shadow-lg' : 
                       answers[q.id] !== undefined ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600' : 'bg-slate-50 dark:bg-slate-800 text-slate-400 hover:bg-slate-100'
                    }`}
                  >
                    {idx + 1}
                  </button>
          ...
          <div className="md:col-span-3 space-y-6">
                       ...
                       return (
                         <button
                           ...
                           className={`w-full group p-6 rounded-3xl border-2 transition-all flex items-center gap-5 text-left ${
                             sel ? 'border-indigo-600 bg-indigo-50 dark:bg-indigo-900/20' : 'border-slate-100 dark:border-slate-800 hover:border-indigo-200'
                           }`}
                         >
                            <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center font-bold ${
                              sel ? 'bg-indigo-600 border-indigo-600 text-white' : 'border-slate-200 dark:border-slate-700 text-slate-300'
                            }`}>
                               {String.fromCharCode(65 + idx)}
                            </div>
                            <span className={`text-lg font-bold ${sel ? 'text-indigo-900 dark:text-white' : 'text-slate-600 dark:text-slate-400'}`}>
                               {opt}
                            </span>
                         </button>
                       )
             ...
             <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] shadow-sm border border-slate-200 dark:border-slate-800 flex justify-between items-center">
                <button 
                  onClick={() => setCurrentIdx(p => p - 1)} disabled={currentIdx === 0}
                ...
                <div className="text-xs font-black text-slate-300 uppercase tracking-widest">{currentIdx + 1} / {questions.length}</div>
                <button 
                  onClick={() => currentIdx === questions.length - 1 ? handleFinalConfirm() : setCurrentIdx(p => p + 1)}
