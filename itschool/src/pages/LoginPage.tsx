...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      // Login - backend avtomatik cookie o'rnatadi
      ...
      const message = err.response?.data?.message || 'Kirish amalga oshmadi';
      setError(message);
    } finally {
      setLoading(false);
    }
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-gradient-to-br from-slate-800 via-slate-900 to-primary-900 flex-col justify-between p-12">
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-16">
            ...
            <span className="text-lg font-black text-white tracking-tight">Ta'lim Platformasi</span>
          </div>
          <h2 className="text-4xl font-black text-white leading-tight tracking-tight">
            O'z bilimingizni<br />
            <span className="text-primary-400">yangi bosqichga</span> olib chiqing.
          </h2>
          <p className="text-slate-400 mt-4 text-sm leading-relaxed max-w-md">
            Zamonaviy ta'lim tizimi orqali o'z maqsadlaringizga erishing. Bir platformada barcha imkoniyatlar.
          </p>
        </div>
        ...
        <div className="relative z-10 bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-5 max-w-xs">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Umumiy Natija</p>
