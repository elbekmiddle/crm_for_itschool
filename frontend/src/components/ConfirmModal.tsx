        ...
        <>
          ...
          <div className="fixed inset-0 flex items-center justify-center p-6 z-[201] pointer-events-none">
            <motion.div
              ...
              className="bg-[var(--bg-card)] border border-[var(--border)] w-full max-w-md rounded-[3rem] shadow-[var(--shadow)] pointer-events-auto overflow-hidden p-10 text-center space-y-8 relative"
            >
              {/* X Close Button */}
              <button 
                onClick={onClose}
                className="absolute top-6 right-6 p-2 text-[var(--text)]/70 hover:text-[var(--text-h)] rounded-xl hover:bg-[var(--hover-bg)] transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="space-y-3">
                <h3 className="text-2xl font-black text-[var(--text-h)] tracking-tight uppercase">{title}</h3>
                <p className="text-[var(--text)] font-bold leading-relaxed">{message}</p>
              </div>
