...
const ExamListPage: React.FC = () => {
  ...
  return (
    <div className="page-container space-y-6 pb-20 lg:pb-6 animate-in">
      <div>
        <h1 className="text-2xl font-black text-slate-900 dark:text-slate-100">Imtihonlar</h1>
        <p className="text-slate-400 text-sm mt-1">Barcha imtihonlaringiz</p>
      </div>

      {/* Filters with sliding background */}
      <div className="flex gap-2 bg-slate-100 dark:bg-slate-800/50 rounded-xl p-1 relative overflow-hidden">
        {tabs.map(({ id, label }) => {
          ...
          return (
            <button
              key={id}
              onClick={() => setTab(id)}
              className={`relative flex-1 py-2 px-3 rounded-lg text-sm font-bold transition-all z-10 ${isActive ? 'text-slate-800 dark:text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}
            >
              {isActive && (
                <motion.div
                  layoutId="active-tab"
              ...
              <span className="relative z-20 flex items-center justify-center gap-1.5">
                {label}
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-black transition-colors ${isActive ? 'bg-primary-100 text-primary-600' : 'bg-slate-200 dark:bg-slate-700 text-slate-400'}`}>
