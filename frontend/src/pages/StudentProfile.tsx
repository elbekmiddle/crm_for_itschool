...
import {
  ArrowLeft, User, GraduationCap, Calendar, Mail, Phone,
  BarChart3, CheckCircle, DollarSign, Sparkles, Loader2
} from 'lucide-react';
    ...
    <div className="page-container animate-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-br from-amber-400 to-orange-500 rounded-2xl p-6 text-white">
            ...
            <p className="text-sm font-semibold leading-relaxed opacity-95">
              "{student.first_name} davomati barqaror. Imtihon ko'rsatkichlari yaxshi — davom eting! 🏆"
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="card p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-black text-slate-800">{totalPaid.toLocaleString()}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Exam Performance (placeholder chart) */}
          <div className="card p-6">
            <h3 className="section-title mb-4">Imtihon Natijalari</h3>
            <div className="flex items-end gap-2 h-32">
              {[65, 78, 82, 88, 75, 92, 85, 90].map((score, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div
                    className={cn("w-full rounded-t-md", score >= 80 ? "bg-green-300" : score >= 60 ? "bg-amber-300" : "bg-red-300")}
                    style={{ height: `${score}%` }}
                  />
            ...
            <div className="flex gap-6 mt-3">
              <div className="flex items-center gap-2 text-xs"><div className="w-3 h-3 rounded bg-green-300" /><span className="text-slate-500 font-semibold">80%+</span></div>
              <div className="flex items-center gap-2 text-xs"><div className="w-3 h-3 rounded bg-amber-300" /><span className="text-slate-500 font-semibold">60-79%</span></div>
              <div className="flex items-center gap-2 text-xs"><div className="w-3 h-3 rounded bg-red-300" /><span className="text-slate-500 font-semibold">&lt;60%</span></div>
            </div>
          </div>
